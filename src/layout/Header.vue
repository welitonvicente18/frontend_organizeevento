<template>
    <header
        ...
        <div class="flex items-center ">
            ...
            <div class="relative flex flex-col items-center justify-center mx-2">
                <div class="">
                ...
                <transition enter-active-class="transition duration-150 ease-out transform"
                    ...
                    <div v-show="dropdownOpen" class="absolute right-0 z-20 w-48 py-2 mt-40 bg-white rounded-md shadow-xl">
                        <span href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white" @click="goUsuarioEdit(userId)">Minha Conta</span>
                        <router-link :to="{ name: 'logout' }" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-whi</router-link>te">
                            Sair
                        </router-link>
                    </div>
                </transition>
...
const userName = ref(store.getters.currentUser?.name || '');
const userId = ref(store.getters.currentUser?.id || '');
